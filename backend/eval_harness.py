"""
Evaluation harness for testing the AI requirements evaluator.

Loads an evaluation dataset, calls Bedrock for each sample,
compares AI output to expected labels, and computes accuracy metrics.
"""
...
def compute_metrics(results: list[dict]) -> dict:
    ...
    for category in ["ambiguity", "testability"]:
        ...
        if total > 0:
            ...
            cat_metrics["precision"] = cat_metrics["tp"] / (cat_metrics["tp"] + cat_metrics["fp"]) if (cat_metrics["tp"] + cat_metrics["fp"]) > 0 else 0
            cat_metrics["recall"] = cat_metrics["tp"] / (cat_metrics["tp"] + cat_metrics["fn"]) if (cat_metrics["tp"] + cat_metrics["fn"]) > 0 else 0
    
    # Completeness accuracy
    comp_total = metrics["completeness"]["within_threshold"] + metrics["completeness"]["outside_threshold"]
    if comp_total > 0:
        metrics["completeness"]["accuracy"] = metrics["completeness"]["within_threshold"] / comp_total
